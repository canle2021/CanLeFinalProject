...
const EditQuote = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const posting = await fetch(`/api/update-quote`, {
        method: "PATCH",
      ...
      if (converToJson.status === 200) {
        alert(
          "THANK YOU! You successfully edit your quote. Please refresh 'My page' to see the update"
        );
    ...
    <SignUpPageDiv>
      ...
      <FormDiv>
        <Form onSubmit={handleSubmit}>
          ...
          <HeadLine>Your current quote: {userProfile.quote}</HeadLine>
          <HeadLine>New quote:</HeadLine>
          <Input
